...
generate_extra_settings () {
    for line in $EXTRA_SETTINGS; do
        echo "$line"
    done
}
...
generate_federation_whitelist () {
    if [ "$DISABLE_FEDERATION" = "true" ]; then
        # Empty federation whitelist.
        cat << EOF
...
generate_registration_requirements () {
    if [ "$REGISTRATION_REQUIRES_EMAIL" = "1" ]; then
        cat << EOF
...
generate_allowed_3pid_patterns () {
    if [ "$RESGISTRATION_ALLOWS_EMAIL_PATTERN" != "" ]; then
        cat << EOF
...
generate_app_service_config_files () {
    if [ "$APP_SERVICE_CONFIG_FILES" != "" ]; then
    cat << EOF
...
generate_server_notices () {
    if [ "$ENABLE_SERVER_NOTICES" = "1" ]; then
    cat << EOF
server_notices:
...
cat << EOF
######################################################
# THIS FILE IS CDIST-GENERATED. DO NOT EDIT BY HAND. #
######################################################

## Extra settings, if any ##
...
# This is also the last part of your UserI1
#
...
email:
   ...
   smtp_host: "$SMTP_HOST"
   smtp_port: $SMTP_PORT # SSL: 465, STARTTLS: 587
   smtp_user: "$SMTP_USER"
   smtp_pass: $SMTP_PASS
   require_transport_security: $SMTP_TLS
   notif_from: "%(app)s <$SMTP_USER>"
   app_name: Matrix

   # Enable email notifications by default
   #
   notif_for_new_users: $ENABLE_NOTIFICATIONS_BY_DEFAULT

   # Defining a custom URL for Riot is only needed if email notifications
   # should contain links to a self-hosted installation of Riot; when set
   # the "app_name" setting is ignored
   #
   riot_base_url: "$RIOT_BASE_URL"
